...
const CreateQuiz = () => {
    ...
    const addQuestion = async (e) => {
        ...
        if (correctOption == undefined) {
            ...
            axios.post("https://elearning-w-api.onrender.com/examquestions/", newQuestion).then((response) => {
    ...
    const handleOptions = ({ data, inputOption }) => {
        ...
        for (let i = 0; i < inputOption.length; i++) {
            ...
            console.log(option);
            axios.put("https://elearning-w-api.onrender.com/examquestions/" + data, option).then((response) => {
                console.log(response.status);
    ...
    const deleteQuestion = (propId) => {
        axios.delete('https://elearning-w-api.onrender.com/examquestions/' + propId).then((response) => {
            console.log(response.status);
                    ...
                    <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 650 }} aria-label="simple table">
                            ...
                            <TableHead>
                                <TableRow>
                                    <Popup
                                        trigger={<CustomButton style={{ marginLeft: "-65%",marginTop:"3%" }}>Add Question</CustomButton>}
                                        modal
                                        nested
                                    >
                                        {close => (
                                            <div style={{ fontSize: "12px", backgroundColor: "Gray", width: "800px", height: "auto" ,borderRadius:"12px",marginTop:"5%"}}>
                                                ...
                                                <form onSubmit={addQuestion} style={{ width: "100%", padding: "10px 5px" }}>
                                                    ...
                                                    <div style={{ width: "100%", padding: "10px 5px", margin: "auto", textAlign: "center" }}>
                                                        <Popup
                                                            trigger={<Button className="formQButton" type='submit' style={{ width: "30%", marginRight: "10px",backgroundColor: "rgb(222, 91, 109)" }}> Confirm </Button>}
                                                            position="top center"
                                                            nested
                                                        >
                                                        </Popup>
                                                        <Button
                                                            className="formQButton" onClick={() => { close(); }} style={{ width: "30%",   backgroundColor: "#478ba2", color: "#fff" }}> Close
                                                        </Button>
                                                    </div>
                                                </form>
                                            </div>
